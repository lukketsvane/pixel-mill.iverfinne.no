...
test('complete editor flow: empty canvas, multi-upload, placement, physics mode, save/open, ZIP and validation',async()=>{
 ...
 ids.get('save').onclick();assert.equal(downloads.length,0);assert.match(ids.get('status').textContent,/name before saving/);
 ids.get('project-name').value='Editor test';ids.get('project-name').onchange({target:ids.get('project-name')});ids.get('project-name').blur();
