...
test('complete editor flow: empty canvas, multi-upload, placement, physics mode, save/open, ZIP and validation',async()=>{
 ...
 const downloadCount=downloads.length;ids.get('save').onclick();assert.equal(downloads.length,downloadCount);ids.get('project-name').value='Editor test';ids.get('project-name').onchange({target:ids.get('project-name')});await Promise.resolve();ids.get('save').onclick();const saved=downloads.at(-1);assert.ok(saved.name.endsWith('.json'));const project=JSON.parse(await saved.blob.text());assert.equal(project.assets.length,4);assert.equal(project.objects.length,3);
...
test('ChatGPT handoff works without an API key and MCP edits reach the live editor',async()=>{
 ...
 try{
  ...
  await ids.get('agent-disconnect').onclick();assert.equal(ids.get('agent-connected').hidden,true);assert.equal((await fetch(url,{method:'POST',body:'{}'})).status,410);
