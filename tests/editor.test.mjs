...
for(const m of html.matchAll(/<([a-z]+)\b([^>]*)>/g)){const e=new Element(m[1]),a=m[2];for(const d of a.matchAll(/data-([a-z]+)="([^"]+)"/g))e.dataset[d[1]]=d[2];const id=a.match(/\bid="([^"]+)"/)?.[1];if(id){e.id=id;ids.set(id,e)}e.value=a.match(/value="([^"]+)"/)?.[1]||'';e.checked=/\bchecked\b/.test(a);e.hidden=/\bhidden\b/.test(a);all.push(e)}
...
globalThis.document={querySelector:s=>ids.get(s.slice(1)),querySelectorAll:s=>all.filter(e=>e.dataset[s.match(/data-(\w+)/)?.[1]]),createElement:t=>new Element(t),createDocumentFragment:()=>new Element(),body:new Element(),addEventListener:(k,f)=>docListeners[k]=f,modelContext:{registerTool:t=>tools.set(t.name,t)}};
...
test('complete editor flow: empty canvas, multi-upload, placement, physics mode, save/open, ZIP and validation',async()=>{
 ...
 const before=JSON.stringify((await read()).objects);ids.get('play').onclick();assert.equal((await read()).playing,true);frame(100);frame(120);ids.get('play').onclick();assert.equal((await read()).playing,false);assert.equal(JSON.stringify((await read()).objects),before);
