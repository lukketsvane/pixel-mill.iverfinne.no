...
class Bucket{constructor(){this.data=new Map();this.n=0}async get(k){const v=this.data.get(k);return v?{etag:v.etag,json:async()=>JSON.parse(v.text)}:null}async delete(keys){for(const k of Array.isArray(keys)?keys:[keys])this.data.delete(k)}async put(k,text,{onlyIf}={}){const v=this.data.get(k);if(onlyIf?.etagMatches&&v?.etag!==onlyIf.etagMatches||onlyIf?.etagDoesNotMatch==='*'&&v)return null;const etag=String(++this.n);this.data.set(k,{text,etag});return{etag}}}
...
test('named saves, reversible deletion, stale writers and room identity use one project',async t=>{
 ...
 const linked=await (await fetch('/api/rooms',{method:'POST',body:JSON.stringify({projectToken:token,revision:save.record.revision})})).json();const roomToken=linked.token;await save.pull();const cloud=await (await fetch('/api/projects/'+token)).json();assert.equal(cloud.roomToken,roomToken);
 const reopened=new Autosave(api,cache);await reopened.init({roomToken});t.after(()=>{save.stop();reopened.stop()});assert.equal(reopened.record.id,id);assert.equal(reopened.record.token,token);assert.equal((await cache.list()).length,1);
 ...
 response=await fetch('/api/projects/'+token,{method:'PUT',body:JSON.stringify({revision:cloud.revision,project:state})});assert.equal(response.status,410);
...
test('incoming edits wait for gestures and never replace a newer applied revision',async()=>{
 ...
 await shared.accept({revision:1,project:project('Old'),canUndo:false});assert.equal(state.name,'From GPT');assert.match(statuses.at(-1),/^Live/);shared.detach();
});