...
test('autosave persists to cloud, restores offline edits and forks conflicts without losing either version',async()=>{
 ...
 try{
  ...
  project.name='Local preserved';restored.changed();await restored.flush();assert.equal(restored.record.token,token);assert.equal(restored.dirty,true);assert.match(restored.blocked,/Conflict/);assert.equal(restored.record.project.name,'Local preserved');await restored.start(project);await restored.flush();assert.notEqual(restored.record.token,token);assert.equal((await(await fetch('/api/projects/'+token)).json()).project.name,'Other device');assert.equal((await(await fetch('/api/projects/'+restored.record.token)).json()).project.name,'Local preserved');
