...
async function project(page){const pending=page.waitForEvent('download');await page.evaluate(()=>document.getElementById('save').click());const file=await pending,stream=await file.createReadStream(),chunks=[];for await(const chunk of stream)chunks.push(chunk);return JSON.parse(Buffer.concat(chunks).toString())}
