...
test('browser: direct sketching, asset references, ZIP handoff and returned-file application work through mobile controls', {skip:!process.env.PIXEL_MILL_BROWSER,timeout:90000},async()=>{
 ...
 try{
  ...
  await page.locator('#import-button').click();assert.equal(await page.locator('#import-dialog').isVisible(),true);assert.equal(await page.locator('#import-kind').inputValue(),'sheet');assert.equal(await page.locator('#import-cell-size').inputValue(),'16');
