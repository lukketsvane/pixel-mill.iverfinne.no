export function installChat(api){
 ...
 function connectChatGPT(){close();api.connectChatGPT()}
 async function open(){panel.hidden=false;$('chat-button').setAttribute('aria-pressed','true');if(opened){if(!enabled)connectChatGPT();return}opened=true;send.disabled=true;
  try{const r=await fetch('/api/chat/status');if(!r.ok)throw Error('Chat is unavailable.');enabled=(await r.json()).enabled;note('');if(enabled){const history=await api.history();for(const m of history)add(m.role,m.content)}else connectChatGPT()}catch(error){note(error.message);opened=false}finally{send.disabled=!enabled}
 }
 function close(){panel.hidden=true;$('chat-button').setAttribute('aria-pressed','false')}
 $('chat-button').onclick=()=>panel.hidden?open():close();$('close-chat').onclick=close;$('chatgpt-button').onclick=connectChatGPT;
