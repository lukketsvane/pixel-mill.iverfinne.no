...
export function installWorkspaces(api){
 ...
 function updateImport(){const pieces=$('import-kind').value==='pieces';for(const el of document.querySelectorAll('.import-advanced'))el.hidden=!pieces}
 ...
 function renderAssets(){
  ...
  for(const a of assets){
   ...
   size.textContent=a.spriteSheet?`${a.spriteSheet.type==='character'?'Character':'Environment'} · ${a.spriteSheet.groups.length} ${a.spriteSheet.type==='character'?'animations':'sets'} · ${a.spriteSheet.frames.length} ${a.spriteSheet.type==='character'?'frames':'assets'}`:`${a.w} × ${a.h} · ${roleNames[a.sheet?.role]||'Asset'}`;
