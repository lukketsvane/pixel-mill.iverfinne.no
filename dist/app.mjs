...
import {detectSpriteGrid,groupSpriteSheet,exportSpriteSheet} from './spritesheets.mjs';
...
async function addFiles(files){if(importing||!files.length)return;importing=true;$('#import-dialog').close();$('#import-button').disabled=true;try{const options={sheet:$('#import-kind')?.value==='sheet',tileSize:Number($('#import-cell-size')?.value)||16,remove:$('#background').value==='auto',scale:Number($('#scale').value),tolerance:clamp(Number($('#tolerance').value)||0,0,100),minArea:clamp(Number($('#min-area').value)||1,1,1000),split:$('#split').checked};const result=await importImages([...files],options,t=>message(t,0));if(options.sheet)for(let i=0;i<result.assets.length;i++){try{const detected=await detectSpriteGrid(result.assets[i]);if(detected.confidence>=.8&&detected.rows>1&&detected.cols>1)result.assets[i]=await groupSpriteSheet(result.assets[i],detected)}catch{}}if(state.assets.length+result.assets.length>2000)throw Error('The palette is full. Open a new level.');await cacheAssets(result.assets);if(result.assets.length){checkpoint();state.assets.push(...result.assets);renderPalette();if(options.sheet){workspaces?.focus?.(result.assets[0].id);workspaces?.show('assets')}else showAssets(true);refresh()}message(result.failed.length?result.failed.join(' · '):`${result.assets.length} pieces` ,result.failed.length?7000:2500)}catch(error){message(error.message)}finally{importing=false;$('#import-button').disabled=false}}
