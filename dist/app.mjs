...
let blockHSB={h:0,s:0,b:87},collisionWidth=8,maskWidth=12,maskEdit=null,adjustBefore=null,assetPress=null;
...
function renderPalette(){const fragment=document.createDocumentFragment();for(const a of state.assets){const b=document.createElement('button');b.title=a.name;b.setAttribute('aria-label',`Place ${a.name}; hold for actions`);b.setAttribute('aria-pressed',String(brush===a.id));const img=document.createElement('img');img.src=a.src;img.alt='';img.draggable=false;b.append(img);let held=false,start=null,timer=null;b.onpointerdown=e=>{held=false;start={x:e.clientX,y:e.clientY};timer=setTimeout(()=>{held=true;assetPress=a.id;$('#asset-action-name').textContent=a.name;$('#delete-asset').textContent=state.objects.some(o=>o.asset===a.id)?'Delete asset and placed copies':'Delete asset';$('#asset-actions').hidden=false},500)};b.onpointermove=e=>{if(start&&Math.hypot(e.clientX-start.x,e.clientY-start.y)>9)clearTimeout(timer)};b.onpointerup=b.onpointercancel=()=>{clearTimeout(timer);start=null};b.onclick=()=>{if(held){held=false;return}$('#asset-actions').hidden=true;brush=a.id;setTool('stamp');showAssets(true)};fragment.append(b)}$('#asset-items').replaceChildren(fragment)}
$('#assets-button').onclick=()=>showAssets($('#assets').hidden);$('#close-assets').onclick=()=>showAssets(false);
$('#reset-assets').onclick=()=>{assetRows=1;updateTray();saveViewPrefs();$('#asset-actions').hidden=true};
...
for(const key of ['hue','saturation','brightness','black','white']){const field=$('#adjust-'+key);field.oninput=e=>{const o=current();if(!o?.asset||o.locked)return;if(!adjustBefore)adjustBefore=snapshot(state);const value=Number(e.target.value);o.adjust={hue:0,saturation:0,brightness:0,black:0,white:255,...o.adjust,[key]:value};if(o.adjust.white<=o.adjust.black){if(key==='black')o.adjust.white=Math.min(255,value+1);else o.adjust.black=Math.max(0,value-1);$('#adjust-'+(key==='black'?'white':'black')).value=o.adjust[key==='black'?'white':'black']}};field.onchange=()=>{if(adjustBefore){if(JSON.stringify(adjustBefore)!==JSON.stringify(snapshot(state)))checkpoint(adjustBefore);adjustBefore=null}refresh(true)}}
$('#reset-adjustments').onclick=()=>changePiece(o=>{delete o.adjust;$('#dimensions').hidden=false});
