...
import {agentTools,editProject,projectInfo,simulate,imageProject,spriteSheetProject,inspectSpriteSheet,exportProjectSpriteSheet,getProjectAssetImage} from './agent.mjs';
...
import {installWorkspaces} from './workspaces.mjs';
import {installSpriteWorkspace} from './sprite-workspace.mjs';
...
let input=null,playControls=null,shared=null,autosave=null,localRevision=0,workspaces=null,spriteWorkspace=null;
let sketchRole='platform',sketchColor='#91e8c1',regionIds=null;
...
let blockHSB={h:153,s:38,b:91},collisionWidth=8,maskWidth=12,maskEdit=null,adjustBefore=null,colorScope='selected',assetPress=null;
...
function blockColor(){return sketchColor}
function setSketchColor(color){sketchColor=color;const [r,g,b]=[1,3,5].map(i=>parseInt(color.slice(i,i+2),16)/255),hi=Math.max(r,g,b),lo=Math.min(r,g,b),d=hi-lo;const h=!d?0:hi===r?((g-b)/d+6)%6:hi===g?(b-r)/d+2:(r-g)/d+4;blockHSB={h:Math.round(h*60)%360,s:Math.round(hi?d/hi*100:0),b:Math.round(hi*100)};updateBlockPicker();saveViewPrefs()}
...
updateBlockPicker();
for(const [key,id] of [['h','hue'],['s','saturation'],['b','brightness']])$('#block-'+id).oninput=e=>{blockHSB[key]=Number(e.target.value);sketchColor=hsbToHex(blockHSB.h,blockHSB.s,blockHSB.b);updateBlockPicker();workspaces?.refresh();saveViewPrefs()};
...
document.addEventListener('touchmove',e=>{if(!e.target.closest('#asset-items,.panel,dialog,#assets-workspace,#sheet-detail'))e.preventDefault()},{passive:false});
...
function setTool(next){if(input?.active())input.cancel();cropping=false;$('#context-menu').hidden=true;$('#block-color').hidden=true;tool=next;$('#brush-options').hidden=!['mask','collision'].includes(next);if(!$('#brush-options').hidden){$('#brush-size').value=next==='mask'?maskWidth:collisionWidth;$('#brush-value').textContent=$('#brush-size').value+' px'}brush=next==='stamp'?brush:null;if(next!=='mask')selected=null;$$('[data-tool]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.tool===next)));$('#hint').textContent=next==='block'?'Draw '+sketchRole+' shapes':next==='region'?'Drag around the area to treat':next==='collision'?'Paint collision · hidden in game':next==='mask'?'Draw over pixels to erase · select an asset first':next==='spawn'?'Tap to place Max':next==='stamp'?'Tap to place · Esc to select':'';canvas.style.cursor=next==='pan'?'grab':next==='select'?'default':'crosshair';refresh();renderPalette()}
...
async function cacheAssets(assets,{guard=()=>true}={}){
 const byId=new Map(assets.map(a=>[a.id,a]));
 const decoded=await Promise.all(assets.map(async a=>{
  const ids=[...new Set((a.spriteSheet?.frames||[]).flatMap(f=>[f.sourceAssetId,f.artwork?.asset]).filter(Boolean))];
  const key=[a.src,JSON.stringify(a.spriteSheet||null),...ids.map(id=>byId.get(id)?.src)],prior=imageSources.get(a.id);
  if(images.has(a.id)&&prior&&key.length===prior.length&&key.every((part,i)=>part===prior[i]))return;
  const src=a.spriteSheet?(await exportSpriteSheet(a,{},assets)).src:a.src,img=await decode(src);
  return{id:a.id,img,key};
 }));
 if(!guard())return;for(const entry of decoded)if(entry){images.set(entry.id,entry.img);imageSources.set(entry.id,entry.key)}
}
...
function renderPalette(){
 const fragment=document.createDocumentFragment(),groups=new Map(),children=new Set(state.assets.flatMap(a=>(a.spriteSheet?.frames||[]).flatMap(f=>[f.sourceAssetId,f.artwork?.asset]).filter(Boolean))),roots=state.assets.filter(a=>!a.hidden&&!children.has(a.id));
 for(const asset of roots){const match=asset.name.match(/^(.*?)[-_ ](?:frame[-_ ]?)?(\d{2,})$/i),key=match?.[1]||null;if(key){if(!groups.has(key))groups.set(key,[]);groups.get(key).push(asset)}}
 const shown=new Set();
 function item(a){const b=document.createElement('button');b.title=a.name;b.setAttribute('aria-label',`Place ${a.name}; hold for actions`);b.setAttribute('aria-pressed',String(brush===a.id));const img=document.createElement('img');img.src=images.get(a.id)?.src||a.src;img.alt='';img.draggable=false;b.append(img);let held=false,start=null,timer=null;b.onpointerdown=e=>{held=false;start={x:e.clientX,y:e.clientY};timer=setTimeout(()=>{held=true;assetPress=a.id;$('#asset-action-name').textContent=a.name;$('#delete-asset').textContent=state.objects.some(o=>o.asset===a.id)?'Delete asset and placed copies':'Delete asset';$('#asset-actions').hidden=false},500)};b.onpointermove=e=>{if(start&&Math.hypot(e.clientX-start.x,e.clientY-start.y)>9)clearTimeout(timer)};b.onpointerup=b.onpointercancel=()=>{clearTimeout(timer);start=null};b.onclick=()=>{if(held){held=false;return}$('#asset-actions').hidden=true;brush=a.id;setTool('stamp');showAssets(true)};return b}
 ...
 else{expandedAssetGroup=null;for(const a of roots){const match=a.name.match(/^(.*?)[-_ ](?:frame[-_ ]?)?(\d{2,})$/i),key=match?.[1],series=groups.get(key);if(series?.length>=3){if(shown.has(key))continue;shown.add(key);const sample=series.reduce((best,entry)=>entry.w*entry.h>best.w*best.h?entry:best);const button=document.createElement('button');button.className='asset-group';button.title=key+' · '+series.length+' frames';button.setAttribute('aria-label',`Open ${key}, ${series.length} frames`);const img=document.createElement('img');img.src=images.get(sample.id)?.src||sample.src;img.alt='';button.append(img);const count=document.createElement('span');count.textContent=series.length;button.append(count);button.onclick=()=>{expandedAssetGroup=key;renderPalette()};fragment.append(button)}else fragment.append(item(a))}}
 $('#asset-items').replaceChildren(fragment);
}
$('#assets-button').onclick=()=>showAssets($('#assets').hidden);$('#close-assets').onclick=()=>showAssets(false);
$('#clear-assets').onclick=()=>{if(!state.assets.length)return;if(state.objects.some(o=>o.artwork)){message('Restore artwork to its sketch before clearing the source sheets.');return;}checkpoint();state.objects=state.objects.filter(o=>!o.asset);state.assets=[];images.clear();brush=null;selected=null;expandedAssetGroup=null;$('#asset-actions').hidden=true;renderPalette();showAssets(false);refresh();message('Asset library cleared · Undo to restore')};
$('#delete-asset').onclick=()=>{if(!assetPress)return;if(state.assets.some(a=>a.parentAssetId===assetPress||a.spriteSheet?.frames?.some(f=>f.sourceAssetId===assetPress||f.artwork?.asset===assetPress))){message('This source belongs to a collection. Ungroup it before removing its sources.');return;}if(state.objects.some(o=>o.artwork?.asset===assetPress)){message('This sheet is used by level artwork. Restore the sketch first.');return;}checkpoint();state.assets=state.assets.filter(a=>a.id!==assetPress);state.objects=state.objects.filter(o=>o.asset!==assetPress);images.delete(assetPress);if(brush===assetPress)brush=null;if(!current())selected=null;assetPress=null;$('#asset-actions').hidden=true;renderPalette();refresh()};
...
function namedProject(){if(hasProjectName(state.name))return true;$('#project').hidden=false;const field=$('#project-name');field.value='';field.placeholder='Project name';field.focus();message('Give the project a name before saving.');return false}
function saveNamedProject(){if(!namedProject())return;autosave?.changed();void autosave?.flush();saveProject(state);$('#project').hidden=true}
$('#save').onclick=()=>exportBusy(saveNamedProject);
...
$('#export').onclick=()=>exportBusy(async()=>{try{if(!namedProject())return;message('Exporting…',0);await exportProject(state,images);$('#project').hidden=true;message('ZIP ready')}catch(error){message(error.message)}});
...
async function addFiles(files){if(importing||!files.length)return;importing=true;$('#import-dialog').close();$('#import-button').disabled=true;try{const options={sheet:$('#import-kind')?.value!=='pieces',recognize:$('#import-kind')?.value!=='image',remove:$('#background').value==='auto',scale:Number($('#scale').value),tolerance:clamp(Number($('#tolerance').value)||0,0,100),minArea:clamp(Number($('#min-area').value)||1,1,1000),split:$('#split').checked};const result=await importImages([...files],options,t=>message(t,0));if(state.assets.length+result.assets.length>2000)throw Error('The palette is full. Open a new level.');await cacheAssets(result.assets);if(result.assets.length){checkpoint();state.assets.push(...result.assets);renderPalette();if(options.sheet){workspaces?.focus?.(result.assets[0].id);workspaces?.show('assets')}else showAssets(true);refresh()}message(result.failed.length?result.failed.join(' · '):result.assets.length===1&&result.assets[0].spriteSheet?`${result.assets[0].spriteSheet.type==='character'?'Character':'Environment'} sheet · ${result.assets[0].spriteSheet.groups.length} ${result.assets[0].spriteSheet.type==='character'?'animations':'sets'} · ${result.assets[0].spriteSheet.frames.length} ${result.assets[0].spriteSheet.type==='character'?'frames':'assets'}`:`${result.assets.length} assets` ,result.failed.length?7000:2500)}catch(error){message(error.message)}finally{importing=false;$('#import-button').disabled=false}}
...
function addBlock(x,y,w,h){checkpoint();const o={id:uid(),asset:null,name:sketchRole[0].toUpperCase()+sketchRole.slice(1),role:sketchRole,x:snap(x,grid),y:snap(y,grid),w:Math.max(grid,snap(w,grid)),h:Math.max(grid,snap(h,grid)),kind:sketchRole==='platform'?'solid':'decor',inset:0,flip:false,rotation:0,locked:false,opacity:1,crop:null,color:blockColor()};if(sketchRole==='background')state.objects.unshift(o);else state.objects.push(o);regionIds=null;selected=o.id;refresh();return o}
...
function beginMask(o){if(o.artwork){message('Restore the sketch before editing its source pixels.');return false}const a=state.assets.find(a=>a.id===o.asset),img=images.get(a?.id);if(!a||!img)return false;if(a.spriteSheet){message('Open the sheet in Assets to edit its frames.');return false;}const c=document.createElement('canvas');c.width=a.w;c.height=a.h;c.getContext('2d').drawImage(img,0,0);maskEdit={object:o,asset:a,originalImage:img,canvas:c,copyId:uid(),changed:false};return true}
...
function finishMask(before){const m=maskEdit;if(!m)return;if(m.changed){const asset={...m.asset,id:m.copyId,name:m.asset.name+' edit',src:m.canvas.toDataURL('image/png')};state.assets.push(asset);if(m.asset.name.endsWith(' edit')&&!state.objects.some(o=>o.asset===m.asset.id)&&!state.assets.some(a=>a.spriteSheet?.frames?.some(f=>f.sourceAssetId===m.asset.id||f.artwork?.asset===m.asset.id)))state.assets=state.assets.filter(a=>a.id!==m.asset.id);checkpoint(before);renderPalette();selected=m.object.id;refresh()}else cancelMask();maskEdit=null}
function openContext(id,p){selected=id;cropping=false;const o=current();if(!o)return;refresh();const menu=$('#context-menu');$('#lock-piece').textContent=o.locked?'Unlock':'Lock';$('#piece-opacity').value=Math.round((o.opacity??1)*100);$('#piece-opacity').disabled=!!o.locked;$('#crop-piece').disabled=!o.asset||!!o.artwork||!!o.locked;$('#reset-crop').hidden=!o.crop;$('#reset-crop').disabled=!!o.locked;$$('[data-order]').forEach(b=>b.disabled=!!o.locked);menu.hidden=false;menu.style.left=clamp(p.x,8,width-184)+'px';menu.style.top=clamp(p.y,64,height-(menu.offsetHeight||260)-12)+'px';}
...
input=installGestures(canvas,{
 ...
 preview:g=>gesture=g,hover:p=>hover=p,select:id=>{selected=id;regionIds=null},selectRegion:rect=>{regionIds=state.objects.filter(o=>!o.collisionOnly&&!o.locked).filter(o=>{const b=objectBounds(o);return b.x>=rect.x&&b.y>=rect.y&&b.x+b.w<=rect.x+rect.w&&b.y+b.h<=rect.y+rect.h}).map(o=>o.id);setTool('select');message(regionIds.length+' shapes selected');refresh()},
 closePanels:()=>{for(const p of ['#project','#help','#dimensions','#context-menu','#block-color'])$(p).hidden=true},
...
function drawPiece(o,alpha=1){if(o.collisionOnly){if(!playing)drawCollision(o.points.map(p=>({x:o.x+p.x,y:o.y+p.y})),o.brushWidth);return}drawObject(paint,o,images.get(o.artwork?.asset||o.asset),{alpha,fill:playing?'#ddd':'#242424',stroke:'#888',lineWidth:1/camera.z})}
...
function render(now){requestAnimationFrame(render);const elapsed=lastTime?Math.min((now-lastTime)/1000,.1):0;lastTime=now;
 ...
 if(!playing){const o=current();if(o){ctx.save();applyObjectTransform(ctx,o);ctx.lineWidth=1/camera.z;ctx.strokeStyle='#fff';ctx.strokeRect(0,0,o.w,o.h);if(o.kind!=='decor'){ctx.setLineDash([3/camera.z,3/camera.z]);ctx.beginPath();ctx.moveTo(0,o.inset);ctx.lineTo(o.w,o.inset);ctx.stroke();ctx.setLineDash([])}ctx.fillStyle='#fff';ctx.fillRect(o.w-4/camera.z,o.h-4/camera.z,8/camera.z,8/camera.z);ctx.restore()}
  ...
  if(regionIds){ctx.save();ctx.strokeStyle='#99e2b4';ctx.lineWidth=1/camera.z;for(const id of regionIds){const o=state.objects.find(o=>o.id===id);if(o){ctx.save();applyObjectTransform(ctx,o);ctx.strokeRect(0,0,o.w,o.h);ctx.restore()}}ctx.restore()}
  if(gesture?.type==='region'){const a=gesture.start,b=gesture.end;ctx.save();ctx.fillStyle='#99e2b420';ctx.strokeStyle='#99e2b4';ctx.lineWidth=1/camera.z;ctx.fillRect(a.x,a.y,b.x-a.x,b.y-a.y);ctx.strokeRect(a.x,a.y,b.x-a.x,b.y-a.y);ctx.restore()}
...
async function applyAgentProject(next,{guard=()=>true,remote=true}={}){if(remote&&shared?.api.busy())return false;const revision=localRevision;await cacheAssets(next.assets,{guard:()=>!gesture&&!numberScrub?.active()&&!importing&&revision===localRevision&&guard()});if(gesture||numberScrub?.active()||importing||revision!==localRevision||!guard()||remote&&shared?.api.busy())return false;if(remote)localRevision++;state=next;if(!current())selected=null;if(playing)colliders=platforms(state.objects);refresh();renderPalette();autosave?.changed()}
...
shared=new SharedLevel({snapshot:()=>snapshot(state),apply:applyAgentProject,busy:()=>deletingProject||!!gesture||!!numberScrub?.active()||importing||!!adjustBefore||!!document.activeElement?.matches?.('#project-name,#inspector input,#inspector select'),
 status:text=>{const el=$('#sync-status');el.textContent=text;el.hidden=!text},pending:value=>{if(autosave?.record){autosave.record.sharedPending=value;void autosave.cacheCurrent()}},
...
async function resumeProjectRoom(){const record=autosave?.record;if(!record?.roomToken)return;if(record.sharedPending){$('#sync-status').hidden=false;$('#sync-status').textContent='Unsent edits · local draft kept';return}try{await shared.connect(record.roomToken);showAgent()}catch(error){message('Saved project opened. '+error.message)}}
async function renderProjects(){const list=$('#saved-projects');list.replaceChildren();for(const p of await autosave?.list()||[]){const row=document.createElement('div');row.className='project-row';const button=document.createElement('button');button.textContent=p.project.name||'Untitled';button.setAttribute('aria-current',String(p.id===autosave.record?.id));button.onclick=async()=>{try{cancelGesture();numberScrub?.cancel();if(playing)togglePlay();await leaveShared();await autosave.load(p.id);await resumeProjectRoom();history=[];future=[];localRevision++;fit();$('#projects-dialog').close()}catch(error){message(error.message)}};const remove=document.createElement('button');remove.textContent='Delete';remove.className='delete-project';remove.setAttribute('aria-label','Delete '+button.textContent);remove.onclick=async()=>{if(remove.dataset.confirm!=='yes'){remove.dataset.confirm='yes';remove.textContent='Delete?';return}remove.disabled=true;deletingProject=true;app.inert=true;try{const current=p.id===autosave.record?.id;if(current){cancelGesture();await shared.queue}if(await autosave.remove(p.id)){await leaveShared();await autosave.start({format:'max-level-studio',version:1,name:'Untitled',spawn:{x:0,y:0},assets:[],objects:[]});history=[];future=[];localRevision++;fit()}await renderProjects();message('Project deleted · Undo delete restores it')}catch(error){message(error.message);remove.disabled=false}finally{deletingProject=false;app.inert=false;shared.wake()}};row.append(button,remove);list.append(row)}if(!list.children.length)list.textContent='No saved projects';$('#undo-project-delete').hidden=!autosave?.lastDeleted}
$('#projects-button').onclick=async()=>{if(importing){message('Finish importing first.');return}try{await renderProjects();$('#project').hidden=true;$('#projects-dialog').showModal()}catch(error){message(error.message)}};
$('#undo-project-delete').onclick=async()=>{try{await autosave.restoreDeleted();await renderProjects();message('Project restored')}catch(error){message(error.message)}};
...
installChat({
 ...
 send:async message=>{
  ...
  if(!shared.connected){if(shared.token)throw Error('Reconnect the agent session first.');await shared.connect();await autosave?.bindRoom(shared.token);window.history.replaceState(null,'','#room='+shared.token)}
  await shared.queue;
  if(gesture||numberScrub.active()||importing)throw Error('Finish the current edit first.');
...
$('#agent-connect').onclick=async()=>{const button=$('#agent-connect');button.disabled=true;try{if(gesture||numberScrub.active()||importing)throw Error('Finish the current edit first.');if(!shared.connected)await shared.connect();await autosave?.bindRoom(shared.token);window.history.replaceState(null,'','#room='+shared.token);showAgent();message('Link ready. Add it in ChatGPT.')}catch(error){message(error.message,6000)}finally{button.disabled=false}};
...
async function executeAgent(name,args={}){
 ...
 if(name==='inspect_sprite_sheet')return inspectSpriteSheet(snapshot(state),args);
 if(name==='export_sprite_sheet'){const out=await exportProjectSpriteSheet(snapshot(state),args);return{width:out.w,height:out.h,name:out.name,src:out.src}};
 if(['group_sprite_sheet','edit_sprite_sheet','group_workspace_sprites','ungroup_sprite_sheet'].includes(name)){if(args.revision!==localRevision)throw Error('Read the current revision first.');const revision=localRevision,edited=await spriteSheetProject(snapshot(state),name,args);await cacheAssets(edited.project.assets,{guard:()=>revision===localRevision&&!gesture});if(revision!==localRevision||gesture)throw Error('The level changed. Try again.');checkpoint();state=edited.project;renderPalette();refresh();return{revision:localRevision,...edited.details}}
 if(name==='get_artwork_request')return{revision:localRevision,request:await getArtworkRequest(snapshot(state),{...args,includeImages:!!args.includeImages})};
 if(name==='get_asset_artwork_request')return{revision:localRevision,request:await getAssetArtworkRequest(snapshot(state),{...args,includeImages:!!args.includeImages})};
 if(['prepare_asset_artwork','apply_asset_artwork','prepare_artwork','apply_artwork','apply_asset_sheet'].includes(name)){
  if(args.revision!==localRevision)throw Error('Read the current revision first.');
  const revision=localRevision,edited=await(name==='prepare_asset_artwork'?prepareAssetArtwork(snapshot(state),{...args,includeImages:!!args.includeImages}):name==='apply_asset_artwork'?applyAssetArtwork(snapshot(state),args):name==='prepare_artwork'?prepareArtwork(snapshot(state),{...args,includeImages:!!args.includeImages}):name==='apply_artwork'?applyArtwork(snapshot(state),args):applyAssetSheet(snapshot(state),args));
  await cacheAssets(edited.project.assets,{guard:()=>revision===localRevision&&!gesture});if(revision!==localRevision||gesture)throw Error('The level changed while processing. Try again.');
  checkpoint();state=edited.project;refresh();renderPalette();return{revision:localRevision,...edited.details,...(edited.request?{request:edited.request}:{})};
 }
 if(name==='get_level')return{revision:localRevision,...projectInfo(snapshot(state)),camera:{...camera},selected,playing,importing};
 if(['import_image','slice_spritesheet','create_spritesheet'].includes(name)){if(args.revision!==localRevision)throw Error('Read the current revision first.');const revision=localRevision,edited=await imageProject(snapshot(state),name,args);await cacheAssets(edited.project.assets,{guard:()=>revision===localRevision&&!gesture});if(revision!==localRevision||gesture)throw Error('The level changed while processing. Try again.');checkpoint();await applyAgentProject(edited.project,{remote:false});return{revision:localRevision,...edited.details,content:edited.image?[edited.image]:[]}}
 if(name==='edit_level'){if(args.revision!==localRevision)throw Error('Read the current revision first.');const next=editProject(snapshot(state),args.operations);checkpoint();await applyAgentProject(next,{remote:false});return executeAgent('get_level')}
 ...
 if(name==='get_asset_image'){const a=await getProjectAssetImage(snapshot(state),args);return{content:[{type:'image',mimeType:'image/png',data:a.src.split(',')[1]}]}}
...
function requestedObjects(){return regionIds??(tool==='select'&&current()?[selected]:undefined)}
function latestArtworkRequest(){return [...(state.artRequests||[])].reverse().find(r=>r.status==='prepared')}
async function workspaceTool(name,args={}){await shared?.queue;const result=await executeAgent(name,{...args,revision:shared?.connected?shared.revision:localRevision});return result.structuredContent||result}
async function mutateWorkspace(fn){cancelGesture();const revision=localRevision,before=snapshot(state),next=structuredClone(before);fn(next);const clean=validateProject(next);await cacheAssets(clean.assets,{guard:()=>revision===localRevision});if(revision!==localRevision)throw Error('The level changed. Try again.');checkpoint(before);state=clean;renderPalette();refresh()}
async function handoffArtwork(args){
 const result=await workspaceTool('prepare_artwork',{...args,includeImages:false}),id=result.request?.id||result.requestId;
 const request=await getArtworkRequest(snapshot(state),{requestId:id,includeImages:true});
 exportArtworkRequest(snapshot(state),request);
 message(shared.connected?'Sketch and sheets ready. Ask ChatGPT to use request '+id+'.':'Sketch and sheets downloaded. Attach the ZIP in ChatGPT; return its PNG here.',7000);
 return request;
}
async function filePNG(file){
 if(file.size>24*1024*1024)throw Error('Choose a PNG under 24 MB.');
...
function assetScope(selection){
 const asset=state.assets.find(a=>a.id===selection?.assetId);if(!asset?.spriteSheet)throw Error('Choose a grouped sheet first.');
 if(selection.frameIds?.length)return{assetId:asset.id,frameIds:selection.frameIds};
 if(selection.groupIds?.length===1)return{assetId:asset.id,groupId:selection.groupIds[0]};
 if(selection.groupIds?.length)return{assetId:asset.id,frameIds:asset.spriteSheet.groups.filter(g=>selection.groupIds.includes(g.id)).flatMap(g=>g.frameIds)};
 return{assetId:asset.id};
}
async function handoffAssetArtwork(selection){
 const result=await workspaceTool('prepare_asset_artwork',{...assetScope(selection),includeImages:false}),id=result.request?.id||result.requestId;
 const request=await getAssetArtworkRequest(snapshot(state),{requestId:id,includeImages:true});exportArtworkRequest(snapshot(state),request);
 message(shared.connected?'Asset selection ready. Ask ChatGPT to use request '+id+'.':'Asset selection downloaded. Attach the ZIP in ChatGPT; return its PNG here.',7000);return request;
}
async function acceptAssetArtworkFile(file,selection){
 const scope=assetScope(selection),pending=(state.assetRequests||[]).filter(r=>r.status==='prepared'),named=pending.find(r=>file.name.includes(r.id));
 const matches=pending.filter(r=>r.target.assetId===scope.assetId&&(scope.groupId?r.target.groupId===scope.groupId:scope.frameIds?r.target.kind==='frames'&&r.target.frameIds.length===scope.frameIds.length&&r.target.frameIds.every(id=>scope.frameIds.includes(id)):r.target.kind==='sheet'));
 if(!named&&matches.length>1)throw Error('Several requests match this selection. Use the request ID as the returned PNG filename.');
 const request=named||matches[0];if(!request)throw Error('Prepare this sheet, animation or frame before applying returned artwork.');
 return workspaceTool('apply_asset_artwork',{requestId:request.id,dataUrl:await filePNG(file),name:file.name.replace(/\.png$/i,'')});
}
async function acceptArtworkFile(file,selectedRequestId){
 const named=(state.artRequests||[]).find(r=>file.name.includes(r.id)),request=named||(state.artRequests||[]).find(r=>r.id===selectedRequestId)||latestArtworkRequest();
 if(!request)throw Error('Prepare the sketch before applying returned artwork.');
 return workspaceTool('apply_artwork',{requestId:request.id,dataUrl:await filePNG(file),name:file.name.replace(/\.png$/i,'')});
}
workspaces=installWorkspaces({getState:()=>state,getObjectIds:requestedObjects,getRequest:latestArtworkRequest,getRole:()=>sketchRole,getColor:blockColor,
 runTool:workspaceTool,mutate:mutateWorkspace,undo:()=>undo(),message,onAssetsRefresh:()=>spriteWorkspace?.refresh(),onGroupSheet:()=>spriteWorkspace?.openProposal(),
 setRole:(role,color)=>{sketchRole=role;setSketchColor(color);regionIds=null;setTool('block')},
 setWorkspace:tab=>{cancelGesture();showAssets(false);if(tab==='assets'&&playing)togglePlay()},
 restoreArtwork:({objectIds})=>mutateWorkspace(s=>{for(const o of s.objects)if(!o.locked&&(!objectIds||objectIds.includes(o.id)))delete o.artwork}),
 prepareArtwork:handoffArtwork,applyArtwork:acceptArtworkFile,applySelectedSheets:args=>workspaceTool('apply_asset_sheet',args)
});
spriteWorkspace=installSpriteWorkspace({getState:()=>state,getAssetId:()=>workspaces.getAssetId(),runTool:workspaceTool,mutate:mutateWorkspace,prepareAssetArtwork:handoffAssetArtwork,applyAssetArtwork:acceptAssetArtworkFile,message});
$('#treatment-scope').onclick=()=>{regionIds=null;selected=null;refresh()};
$('#select-region').onclick=()=>{regionIds=null;setTool('region')};
setTool('block');
...
if(typeof location!=='undefined'){
 ...
 if(typeof indexedDB!=='undefined'){
  ...
  app.inert=true;
  (async()=>{try{await autosave.init({token:project,roomToken:room});await resumeProjectRoom();if(room)fit()}catch(error){message(error.message,6000);if(!autosave.ready){window.history.replaceState(null,'',location.pathname);await autosave.init()}}finally{app.inert=false}})();
  const catchUp=()=>{if(document.visibilityState==='visible')shared.wake()};window.addEventListener('focus',catchUp);window.addEventListener('online',catchUp);document.addEventListener('visibilitychange',catchUp);
