...
export function validateSpriteSheet(raw,asset,assets){
 ...
 if(result.rows*result.cols>sheetLimit||result.marginX+result.cols*result.cellWidth+(result.cols-1)*result.gutterX>asset.w||result.marginY+result.rows*result.cellHeight+(result.rows-1)*result.gutterY>asset.h)throw Error('Sprite grid exceeds its source image or 8,192 cells.');
 ...
 result.frames=raw.frames.map(frame=>{
  ...
  if(result.source==='sheet'&&!sourceAssetId){const x=result.marginX+col*(result.cellWidth+result.gutterX),y=result.marginY+row*(result.cellHeight+result.gutterY);if(item.x!==x||item.y!==y||item.w!==result.cellWidth||item.h!==result.cellHeight)throw Error('Sprite frame does not match its source grid.')}
  ...
  if(frame.artwork!==undefined)item.artwork=sheetArtwork(frame.artwork,result,assets,asset);return item;
 });
...
export async function groupSpriteSheet(asset,args={},assets=[]){
 ...
 for(let row=0;row<rows;row++)for(let col=0;col<cols;col++){
  ...
  meta.frames.push(retained?{...sheetCopy(retained),empty}:{id:'frame-'+row+'-'+col,row,col,x,y,w:cellWidth,h:cellHeight,empty});
 }
...
export function editSpriteSheet(asset,operations,assets){
 ...
 for(const op of operations){
  ...
  else if(op.type==='replace_frame'){const frame=meta.frames.find(frame=>frame.id===op.frameId);if(!frame)throw Error('Sprite frame not found.');if(op.artwork===null)delete frame.artwork;else frame.artwork=sheetArtwork(op.artwork,meta,assets,asset)}
  else if(op.type==='replace_group'){const group=getGroup(op.groupId);if(!Array.isArray(op.artworks)||op.artworks.length!==group.frameIds.length)throw Error('Supply one replacement for every group frame.');group.frameIds.forEach((id,n)=>{const frame=meta.frames.find(frame=>frame.id===id);if(op.artworks[n]===null)delete frame.artwork;else frame.artwork=sheetArtwork(op.artworks[n],meta,assets,asset)})}
...
export async function exportSpriteSheet(asset,options={},assets=[]){
 ...
 for(const placement of placements){
  ...
  const crop=art??frame,dx=art?(art.offsetX??0):(frame.offsetX??0),dy=art?(art.offsetY??0):(frame.offsetY??0),dw=art?(art.targetW??meta.cellWidth):frame.w,dh=art?(art.targetH??meta.cellHeight):frame.h;
