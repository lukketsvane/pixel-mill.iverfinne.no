...
export function drawObject(ctx,o,image,{alpha=1,fill='#fff',stroke=null,lineWidth=1}={}){ctx.save();ctx.globalAlpha=alpha*(o.opacity??1);applyObjectTransform(ctx,o);if(image){if(o.flip){ctx.translate(o.w,0);ctx.scale(-1,1)}const crop=o.crop||{x:0,y:0,w:1,h:1},w=image.naturalWidth||image.width,h=image.naturalHeight||image.height;ctx.drawImage(image,crop.x*w,crop.y*h,crop.w*w,crop.h*h,0,0,o.w,o.h)}else{ctx.fillStyle=fill;ctx.fillRect(0,0,o.w,o.h);if(stroke){ctx.strokeStyle=stroke;ctx.lineWidth=lineWidth;ctx.strokeRect(0,0,o.w,o.h)}}ctx.restore()}
...
export class TwoFingerTransform{
 ...
 update(a,b){const now=pair(a,b);if(now.distance<8){this.ready=false;this.base={...this.target};this.turn=0;return}
  ...
  if(this.kind==='camera'){this.target.z=clamp(this.base.z*now.distance/this.start.distance,.25,12);this.target.rotation=wrap((this.base.rotation||0)+this.turn);anchorCamera(this.target,this.anchor,now.center);return}
  ...
  const offset=rotate({x:(b0.w/2-local.x)*sx,y:(b0.h/2-local.y)*sy},radians(b0.rotation||0)+this.turn);
  this.target.w=b0.w*sx;this.target.h=b0.h*sy;this.target.x=mid.x+offset.x-this.target.w/2;this.target.y=mid.y+offset.y-this.target.h/2;this.target.rotation=degrees(wrap(radians(b0.rotation||0)+this.turn));this.target.inset=clamp((b0.inset||0)*sy,0,this.target.h-1);
 }
}
